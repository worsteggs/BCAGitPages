...
export default function(json, divId, xField, yField, margin2, cb) {
  ...
  divObj.innerHTML = ''
  const colors = ['#ff77a1', '#ff6abf', '#e96ef5', '#826aff', '#06b3f9', '#0eca81', '#51d932', '#a0d33c', '#d9c122']
  ...
  sumstat.sort((a, b) => { a.median - b.median })
  ...
  svg.append('g')
    .call(d3.axisLeft(y))
    .selectAll('text')
    .datum(function(d) { return splitStr(d) })
    .selectAll('tspan')
    .data(function() { const text = d3.select(this).text(); d3.select(this).text(''); return splitStr(text, 15) })
    .enter()
    .append('tspan')
    .attr('x', function(d) { return d3.select(this.parentNode).attr('x') })
    .attr('dy', function(d, i) { return (i == 0 ? 0.4 : 1) + 'em' })
    .text(d => d)
  ...
  svg
    ...
    .attr('d', function(e) { return pathLine(e.points, e.key) })
    .attr('stroke', '#000a')
